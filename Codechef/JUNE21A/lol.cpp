...
int32_t main(){
	...
	for(int i=0; i<k; i++){
		cin >> arr[i];
		if(arr[i]-(base)(r-1) >= 1)
			evalpoints.insert(arr[i]-(base)(r-1));
	}
	for(int i=0; i<k; i++){
		cin >> prob[i];
	}
	...
	map<int, int> mp;
	for(int i=0; i<evalpts.size(); i++){
		mp[evalpts[i]+(base)(r-1)] = ncrvals[i];
	}
	...
	dp[0][1] = prob[0]*includeVal[0] % mod;

	for(int i=1; i<k; i++){
		dp[i][1] = (prob[i]*(dp[i-1][0]+dp[i-1][1]))%mod * includeVal[i];
		dp[i][0] = ((1-prob[i])%mod + mod)%mod * (dp[i-1][0]+dp[i-1][1]);
		dp[i][1] %= mod;
		dp[i][0] %= mod;
	}
